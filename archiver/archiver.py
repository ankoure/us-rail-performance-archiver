    ...
    def archive_one(self, feed: Feed):
        ...
        try:
            with self.telemetry.span("feed.poll", tags={"feed": feed.name}):
                ...
                http = feed.client.get(feed.path, headers=conditional_headers)

                if http.status_code == 304:
                    self.telemetry.incr("feed.not_modified", tags={"feed": feed.name})
                    ...
                    if current_digest == prior.last_digest:
                        self.telemetry.incr("feed.duplicate", tags={"feed": feed.name})
