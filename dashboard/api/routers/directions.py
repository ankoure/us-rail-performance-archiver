...
def get_directions(agency: Agency = Depends(dependencies.get_agency)) -> list[dict]:
    ...
    agency, resolved from the latest static-GTFS snapshot any of its feeds has
    landed. directions.txt is an MBTA GTFS extension -- most other agencies'
    feeds omit it, so this comes back empty for those, not an error."""
    if not agency.feed_names:
        raise HTTPException(
            status_code=404, detail=f"Agency {agency.agency_id!r} has no feeds"
        )
    table = data.read_latest_version_mart("gtfs_directions", agency.feed_names[0])
