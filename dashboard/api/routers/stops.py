...
def get_stops(agency: Agency = Depends(dependencies.get_agency)) -> list[dict]:
    ...
    the latest static-GTFS snapshot any of its feeds has landed. All of an
    agency's feeds share the same underlying schedule, so the first is enough."""
    if not agency.feed_names:
        raise HTTPException(
            status_code=404, detail=f"Agency {agency.agency_id!r} has no feeds"
        )
    table = data.read_latest_version_mart("gtfs_stops", agency.feed_names[0])
